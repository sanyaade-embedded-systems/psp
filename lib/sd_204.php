...
// $psp_id sd_204.php date 20160308
// PBH-204 basic library
...
function dio_out($pin, $type)
{
	if(($pin < DO_0) || ($pin > DI_3))
		exit("dio_out: pin number out of range $pin\r\n");

	$pid = pid_open_nodie("/mmap/io4", "dio_out");

	if(($pin >= DO_0) && ($pin <= DO_3))
	{
		if(!pid_ioctl($pid, "get 7 mode"))
			pid_ioctl($pid, "set 7 mode out low"); // enable 74HC245 output
	...
	switch($type)
	{
		...
		case TOGGLE:
			...
			break;
		default:
			exit("dio_out: invalid output $type\r\n");
			break;
	}
...
{
	if(($pin < DO_0) || ($pin > DI_3))
		exit("dio_in: pin number out of range $pin\r\n");

	$pid = pid_open_nodie("/mmap/io4", "dio_in");

	if(($pin >= DI_0) && ($pin <= DI_3))
	{
		if(!pid_ioctl($pid, "get $pin mode"))
			pid_ioctl($pid, "set $pin mode in");
	}
...
function uart_setup_driver($set)
{
	...
	pid_ioctl($pid, "set 0-3 mode out");

	$io16 = 0;
	pid_read($pid, $io16);

	pid_write($pid, ($io16 & 0xfff0) | $set);
...
function uart_setup($uart_id, $baud, $set = "N81N")
{
	...
	$pid = pid_open_nodie("/mmap/uart0", "uart_setup");
	...
	switch($flowctrl)
	{
		...
		case "D": /* Differential Drive */
			$flowctrl = 3;
			uart_setup_driver(0x02);
			break;
		case "T": /* Differential Drive - TxDE flow control */
			$flowctrl = 3;
			uart_setup_driver(0x0c);
			break;
		default:
...
function uart_read($uart_id, &$rbuf, $rlen = MAX_STRING_LEN)
{
	...
	$pid = pid_open_nodie("/mmap/uart0", "uart_read");
...
function uart_readn($uart_id, &$rbuf, $rlen)
{
	...
	$pid = pid_open_nodie("/mmap/uart0", "uart_readn");
...
function uart_write($uart_id, $wbuf, $wlen = MAX_STRING_LEN)
{
	...
	$pid = pid_open_nodie("/mmap/uart0", "uart_write");
...
function uart_txfree($uart_id)
{
	...
	$pid = pid_open_nodie("/mmap/uart0", "uart_txfree");
